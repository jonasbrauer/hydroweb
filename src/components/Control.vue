...
export default {
  ...
  methods: {
    controlAction() {
      const path = `${Constants.HOST_URL}/devices/${this.device.id}/action`;
      const options = {
    ...
    controlEdit() {
      const path = `${Constants.HOST_URL}/devices/${this.device.id}/controls/${this.control.id}`;
      const options = {
        headers: { 'Content-Type': 'application/json' },
      };
      const data = {
    ...
    controlDelete(id) {
      const path = `${Constants.HOST_URL}/devices/${this.device.id}/controls/${id}`;
      axios.delete(path);
    },
    toggleEdit() {
